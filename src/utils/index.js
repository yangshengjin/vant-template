...
function compareDate(myDate) {
  ...
  if (currentDate !== null && myDate !== null) {
    ...
    var arys1 =
      currentDate.split('/').length > 1
        ? currentDate.split('/')
        : currentDate.split('-')
    var arys2 =
      myDate.split('/').length > 1 ? myDate.split('/') : myDate.split('-')
    var n = 0
    flag = false
    while (n < 3) {
 ...
 *  @param: {dateString} myDate, {String} type -> TODO: type is 'last' or 'next'
 *  @param: {Number} dday 表示要计算前后dday天
 *  @return: {dateString} dateValue
 * */
function calculation(myDate, type, dday = 1) {
...
function getWeekDay(day) {
  ...
  if (day != null) {
    ...
    if (
      day.getDate() === toDate.getDate() &&
      day.getMonth() === toDate.getMonth() &&
      day.getFullYear() === toDate.getFullYear()
    ) {
      weekDay = '今天'
    } else {
